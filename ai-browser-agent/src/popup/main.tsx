...
function App() {
  ...
  const activate = () => { void send("SET_LANGUAGE", language); void send("ACTIVATE_AGENT", text); setText("") }
  return <main className="popup"><header><span className="orb"><Sparkles size={14} /></span><div><strong>GuideDot</strong><small>{domain}</small></div></header><section><span className="avatar active" aria-hidden /><div><b>Visual browser agent</b><p>{status}</p></div></section><label className="language"><Languages size={14} /><select value={language} onChange={(event) => { setLanguage(event.target.value); void send("SET_LANGUAGE", event.target.value) }} aria-label="Agent language"><option value="en-IN">English</option><option value="hi-IN">हिन्दी</option><option value="ta-IN">தமிழ்</option><option value="te-IN">తెలుగు</option><option value="kn-IN">ಕನ್ನಡ</option><option value="bn-IN">বাংলা</option><option value="mr-IN">मराठी</option></select></label><form onSubmit={(event) => { event.preventDefault(); activate() }}><input value={text} onChange={(event) => setText(event.target.value)} placeholder="Ask me what to do…" /><button aria-label="Start guiding" type="submit"><Send size={16} /></button></form><div className="actions"><button className="primary" onClick={activate}><Sparkles size={15} /> Activate agent</button><button aria-label="Use voice" onClick={() => void send("START_LISTENING")}><Mic size={16} /></button><button aria-label="Stop agent" onClick={() => void send("STOP_AGENT")}><Square size={14} /></button></div><footer>Active only in this website tab. You stay in control.</footer></main>
}
